...
def sample_function(user_train, usernum, itemnum, batch_size, maxlen,
                    relation_matrix, result_queue=None):
    def sample(user):
    ...
    if args.num_workers > 1:
        ...
        while True:
            ...
            for i in trange(batch_size):
                ...
                while len(user_train[user]) <= 1: user = np.random.randint(1,
                                                                           usernum + 1)
...
class WarpSampler(object):
    def __init__(self, User, usernum, itemnum, relation_matrix, batch_size=64,
        ...
        if n_workers <= 1:
            ...
            self.data = (User, usernum, itemnum, batch_size, maxlen,
                                  relation_matrix)
    ...
    def next_batch(self):
        if self.n_workers <= 1:
            ...
            res = sample_function(*self.data)
            u, seq, time_seq, time_matrix, pos, neg = [], [], [], [], [], []

            for row in res:
                u.append(row[0])
                seq.append(row[1])
                time_seq.append(row[2])
                time_matrix.append(row[3])
                pos.append(row[4])
                neg.append(row[5])

            return [u, seq, time_seq, time_matrix, pos, neg]
...
def cleanAndsort(User, time_map):
    ...
    for user, items in User_res.items():
        ...
        User_res[user] = list(map(lambda x: [x[0], int(round(
            (x[1] - time_min) / time_scale) + 1)], items))
...
def data_partition(fname):
    ...
    num_lines = sum(1 for line in open('data/%s.txt' % fname, 'r'))
    f = open('data/%s.txt' % fname, 'r')
    for idx_line, line in enumerate(tqdm(f, total=num_lines)):
        try:
    ...
    num_lines = sum(1 for line in open('data/%s.txt' % fname, 'r'))
    # U -> list of (item, time) for each user
    for idx_line, line in enumerate(tqdm(f, total=num_lines)):
        try:
            u, i, rating, timestamp = line.rstrip().split('\t')
        except:
    ...
    for user in tqdm(User, desc='User'):
...
def evaluate(model, dataset, args):
    ...
    for u in users:
        ...
        predictions = -model.predict(
            *[np.array(l) for l in [[u], [seq], [time_matrix], item_idx]])
